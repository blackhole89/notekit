...
LinkOptions            :=  $(shell pkg-config --libs gtkmm-3.0) $(shell pkg-config webkit2gtk-4.0 --libs) $(shell pkg-config --libs gtksourceviewmm-3.0) 
...
CXX      := /usr/bin/g++
CC       := /usr/bin/gcc
CXXFLAGS :=  -g -Wall -O0 $(shell pkg-config --cflags gtkmm-3.0) $(shell pkg-config webkit2gtk-4.0 --cflags) $(shell pkg-config --cflags gtksourceviewmm-3.0)  $(Preprocessors)
